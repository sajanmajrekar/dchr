 ...
 function fetch_data(is_date_search,start_date, end_date, roles, experiance, nperiod)
             {
                       ...
                       $("#leaddatatable_wrapper").fadeIn();
                       setTimeout(function(){ var totalRowCount = 0;
                        var rowCount = 0;
                        var table = document.getElementById("leaddatatable");
                        ...
                        var message = "\nRow Count: " + rowCount;
                        //alert(message);
                            $(".occurance").html("Total "+rowCount+" record selected. <a href=#leaddatatable_wrapper>Show Result</a>");
                            $("#datasource").val(rowCount);
                            var empty = false;
                            $('#leaddatatable td').each(function() {
                              if ($(this).hasClass("dataTables_empty")) {
                                empty = true;
                              }
                            });
                            if ((rowCount === 1 && empty) || rowCount === 0) {
                                $(".occurance").html("No matching records found for the selected filters.");
                                $("#datasource").val("");
                            }
                        }, 1000);
    ...
    $('#search').click(function(){
      ...
      if ((start_date !== '' && end_date === '') || (start_date === '' && end_date !== ''))
      {
       ...
       $.bootstrapGrowl('<p>Please select both From and To dates.</p>', {
            type: growlType,
            delay: 3000,
            allow_dismiss: true,
...
$('#mailform').validate({
                ...
                messages: {
                    'textarea-ckeditor':{
                        required: "Enter mail content"
                    },
                    'subject':{
                         required: "Enter mail subject."
                    },
                    'datasource':{
                        required: "Select the datasource using filters"  
                    },
                    'email-chosen-multiple[]':{
                 ...
                 submitHandler: function(form) {
                                    ...
                                    var r = confirm('Click "OK" to send the mails to selected datasource');
                                    if (r == true) {

...
try {
    ...
    if (($filePath === '' || !is_file($filePath) || !is_readable($filePath)) && isset($connect) && $connect instanceof mysqli) {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                resumeIntelligenceLog('view_resume', 'Failed to execute resume lookup query.', array(
            ...
            resumeIntelligenceLog('view_resume', 'Failed to prepare resume lookup query.', array(
                'resume_file' => $resumeFile,
                'db_error' => isset($connect->error) ? $connect->error : ''
            ));
        }
    }

    if (!is_file($filePath) || !is_readable($filePath)) {
        resumeIntelligenceLog('view_resume', 'Resume file was not found or readable.', array(
            'resume_file' => $resumeFile,
    ...
    if (@readfile($filePath) === false) {
        resumeIntelligenceLog('view_resume', 'readfile failed for resume.', array(
            'resume_file' => $resumeFile,
            'resolved_path' => $filePath
        ));
        http_response_code(500);
        exit('Could not open resume.');
    }
    exit;
} catch (Throwable $e) {
    resumeIntelligenceLog('view_resume', 'Unhandled exception while opening resume.', array(
        'message' => $e->getMessage(),
        'file' => $e->getFile(),
        'line' => $e->getLine(),
        'resume_param' => isset($_GET['file']) ? (string) $_GET['file'] : ''
    ));
    http_response_code(500);
    exit('Could not open resume. Check data/logs/view_resume.log on the server.');
}
